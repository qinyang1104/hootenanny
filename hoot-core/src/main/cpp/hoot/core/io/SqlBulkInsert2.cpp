...
SqlBulkInsert2::SqlBulkInsert2(QSqlDatabase& db, const QString tableName, const QStringList columns) :
_db(db),
...
SqlBulkInsert2::~SqlBulkInsert2()
{
  ...
  if (_pendingCount > 0)
  {
    LOG_WARN(
      ...
      "SqlBulkInsert2. You should call flush before destruction.");
  }
  if (_query)
  {
    _query->finish();
...
void SqlBulkInsert2::_initValsList()
{
  for (int i = 0; i < _columns.size(); i++)
  {
...
void SqlBulkInsert2::insert(const QVariantList& vals)
{
